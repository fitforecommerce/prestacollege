...
<div class="row">
	<div class="col-xs-12">
    <h3><i class="icon-cart-plus"></i> {l s='Fake Orders' mod='prestacollege'}</h3>
    <form id="db_snapshot_form" class="defaultForm form-horizontal"  action="{$form_action_url}" method="post" enctype="multipart/form-data" novalidate="">
      ...
      <input type="hidden" name="PRESTACOLLEGE_ACTION" value="fakeorders">

      {foreach $orderfaker_def as $k => $v}
        {if $k == 'localization'}
          {continue}
        {/if}
          <div class="form-group">
              <label class="control-label col-lg-4">{$orderfaker_labels[$k]}</label>
              <div class="col-lg-4">
